        ...
        public async Task<JsonResult> GuiNhacLichManual()
        {
            ...
            try
            {
                ...
                using (var db = new HamaStudioDbContext())
                {
                    ...
                    if (upcoming.Count == 0)
                        return Json(new { success = true, message = "Không tìm thấy lịch chụp nào vào ngày mai (" + tomorrow.ToString("dd/MM/yyyy") + ") để nhắc." });
    ...
    <div style='max-width: 600px; margin: 0 auto; background-color: #ffffff; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); border-top: 5px solid #8c8574;'>
        ...
        <div style='padding: 40px;'>
            ...
            <div style='margin-top: 40px; padding-top: 20px; border-top: 1px solid #eee; text-align: center;'>
                ...
                return Json(new { success = true, message = "Đã gửi mail nhắc lịch thành công cho " + sentCount + " khách hàng." });
            }
            catch (Exception ex)
            {
