...
-- ✨ NEW TABLE: LichChup (Shooting Schedule)
CREATE TABLE LichChup (
    MaLichChup INT PRIMARY KEY IDENTITY(1,1),
    MaDichVu INT FOREIGN KEY REFERENCES DichVu(MaDichVu),
    NgayChup DATE NOT NULL,
    SoLichToiDa INT NOT NULL,
    SoLichConLai INT NOT NULL,
    GhiChu NVARCHAR(500),
...
ALTER TABLE LichChup ADD CONSTRAINT CHK_SoLichToiDa CHECK (SoLichToiDa > 0);
ALTER TABLE LichChup ADD CONSTRAINT CHK_SoLichConLai CHECK (SoLichConLai >= 0);
GO

-----------------------------------------------------------
-- 3. INDEXES cho Performance
-----------------------------------------------------------
CREATE INDEX IX_LichChup_NgayChup ON LichChup(NgayChup);
CREATE INDEX IX_LichChup_MaDichVu_NgayChup ON LichChup(MaDichVu, NgayChup);
CREATE INDEX IX_DatLich_MaLichChup ON DatLich(MaLichChup);
GO
...
-----------------------------------------------------------
-- 5. INSERT DỮ LIỆU MẪU (10 DÒNG MỖI BẢNG)
-----------------------------------------------------------
...
(1, 1, DATEADD(DAY, 5, CAST(GETDATE() AS DATE)), '08:00 - 17:00', N'Bà Nà Hills', N'Cần ekip mang theo hắt sáng lớn'),
(2, 8, DATEADD(DAY, 6, CAST(GETDATE() AS DATE)), '07:30 - 10:30', N'Bán đảo Sơn Trà', N'Khách thích tone màu phim'),
(3, 3, DATEADD(DAY, 7, CAST(GETDATE() AS DATE)), '18:00 - 22:00', N'Khách sạn Novotel', N'Chụp tiệc Gala Dinner'),
(4, 4, DATEADD(DAY, 10, CAST(GETDATE() AS DATE)), '07:00 - 17:00', N'Trường ĐH Kinh Tế', N'Lớp 40 người'),
(5, 5, DATEADD(DAY, 12, CAST(GETDATE() AS DATE)), '09:00 - 11:30', N'Hama Studio Cơ sở 1', N'Gia đình có em bé 2 tuổi'),
(6, 2, DATEADD(DAY, 15, CAST(GETDATE() AS DATE)), '13:30 - 17:30', N'Hama Studio Cơ sở 1', N'Makeup phong cách trong trẻo'),
(7, 9, DATEADD(DAY, 16, CAST(GETDATE() AS DATE)), '14:00 - 16:00', N'Hama Studio Cơ sở 2', N'Lấy phông nền xám đậm'),
(8, 10, DATEADD(DAY, 20, CAST(GETDATE() AS DATE)), '15:00 - 18:00', N'Cầu Tình Yêu', N'Chụp lúc hoàng hôn'),
(9, 7, DATEADD(DAY, 21, CAST(GETDATE() AS DATE)), '08:00 - 12:00', N'Hama Studio Cơ sở 2', N'Sản phẩm là nước hoa'),
(10, 6, DATEADD(DAY, 25, CAST(GETDATE() AS DATE)), '09:00 - 12:00', N'Nhà khách hàng', N'Nhà chung cư, ánh sáng tự nhiên tốt');
...
-----------------------------------------------------------
-- 6. ✨ INSERT DỮ LIỆU MẪUIỂU BẢNG LICHCUP
-----------------------------------------------------------

-- Insert LichChup (Shooting Schedules)
INSERT INTO LichChup (MaDichVu, NgayChup, SoLichToiDa, SoLichConLai, GhiChu, NgayTao, TrangThai) VALUES
-- Photography Services (MaDichVu 1-5)
(1, DATEADD(DAY, 5, CAST(GETDATE() AS DATE)), 5, 3, N'Studio A - Lighting ready', DATEADD(DAY, -2, GETDATE()), N'Hoạt động'),
(1, DATEADD(DAY, 10, CAST(GETDATE() AS DATE)), 3, 1, N'Studio B - Ngoài trời', DATEADD(DAY, -1, GETDATE()), N'Hoạt động'),
(2, DATEADD(DAY, 15, CAST(GETDATE() AS DATE)), 4, 2, N'Studio A - Pre-wedding setup', DATEADD(DAY, -3, GETDATE()), N'Hoạt động'),
(3, DATEADD(DAY, 7, CAST(GETDATE() AS DATE)), 2, 0, N'Khách sạn Novotel - Event', DATEADD(DAY, -4, GETDATE()), N'Đã đầy'),
(4, DATEADD(DAY, 10, CAST(GETDATE() AS DATE)), 3, 2, N'Trường ĐH Kinh Tế - Kỷ yếu', DATEADD(DAY, -2, GETDATE()), N'Hoạt động'),
...
-- 7. VERIFY DỮ LIỆU
-----------------------------------------------------------
PRINT '✓ Database HamaStudioDB created successfully!'
PRINT '✓ All tables created'
PRINT '✓ LichChup table added'
PRINT '✓ Sample data inserted'
PRINT ''
PRINT '--- Summary Statistics ---'
SELECT 'QuanTriVien' AS TableName, COUNT(*) AS RecordCount FROM QuanTriVien
UNION ALL SELECT 'KhachHang', COUNT(*) FROM KhachHang
UNION ALL SELECT 'DanhMucDichVu', COUNT(*) FROM DanhMucDichVu
UNION ALL SELECT 'DichVu', COUNT(*) FROM DichVu
UNION ALL SELECT 'DatLich', COUNT(*) FROM DatLich
...
UNION ALL SELECT 'DacQuyen', COUNT(*) FROM DacQuyen
UNION ALL SELECT 'DichVuBoSung', COUNT(*) FROM DichVuBoSung
UNION ALL SELECT 'LichChup (✨ NEW)', COUNT(*) FROM LichChup;
